...
def load_quantity_datasets(path = 'data/nli_data_052201.pickle'):
...
def main():
    ...
    pickle_in = open("data/data_052201.pickle", "rb")
    # pickle_in = open("data/original_data.pickle", "rb")
    train, dev, test = pickle.load(pickle_in)

    glove = import_glove('data/snli_vectors_052201.txt')
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == 'repackage':
        repackage_glove('data/glove.6B.50d.txt', 'data/snli_vectors_052201.txt', 'data/snli_1.0/')
    elif len(sys.argv) > 1 and sys.argv[1] == 'store':
        # train, dev, test = load_all_snli_datasets('data/snli_1.0/')
        train, dev, test = load_quantity_datasets()
        pickle_out = open("data/data_052201.pickle", "wb")
